...
<details>
<summary><strong>Astra review には Codex CLI が必要</strong></summary>

`/astra-review` は Claude が書いたコードを GPT-6-Astra に送り、プロバイダーをまたいだ独立したレビューを受けます。ローカルにインストールされた [Codex CLI](https://github.com/openai/codex) を ChatGPT ログイン（`codex login`）で実行します。OpenAI API キーは不要で、環境内の API キーが転送されることもありません。このコマンドを実行することは、その差分を OpenAI に送信することへの同意となります。セットアップ、スコープ、プッシュのゲーティング、トラブルシューティングは [Astra Review ガイド](../ASTRA-REVIEW-GUIDE.md)にあります。
</details>
...
[コマンド クイックリファレンス](./COMMANDS-QUICK-REF.md) | [手動適用ガイド](../MANUAL-ADAPTATION-GUIDE.md) | [Astra Review ガイド](../ASTRA-REVIEW-GUIDE.md) | [トラブルシューティング FAQ](../../TROUBLESHOOTING.md) | [ロードマップ](../ROADMAP.md)
